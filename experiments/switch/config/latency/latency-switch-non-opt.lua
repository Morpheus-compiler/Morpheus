...
local TS_DST_MAC	= "12:34:56:78:90:01"
local TS_SRC_IP	    = "10.0.0.1"
local TS_DST_IP		= "10.0.0.10"
local TS_SRC_PORT	= 1234
local TS_DST_PORT	= 319

local TS_OUTER_SRC  = "172.16.211.5"
...
function txTimestamper(queue, time, tsLoad)
	...
	local rateLimit = timer:new(tsLoad) -- 10kpps timestamped packets
	local runtime = nil
	if time > 0 then
...
function rxTimestamper(queue)
	...
	while mg.running() do
		...
		for i = 1, numPkts do
				...
				local txTs = getTimestampFromPacket(bufs[i], 14+20+8+6)
	...
	local f = io.open("switch-latency-non-opt.txt", "w+")
