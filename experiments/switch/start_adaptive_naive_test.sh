...
NAIVE_OVERHEAD_INSTRUMENTATION_CONFIG_FILE="${DUT_REMOTE_MORPHEUS_FOLDER}"/experiments/switch/config/adaptive_vs_naive/morpheus-naive-overhead.yaml
ADAPTIVE_OVERHEAD_INSTRUMENTATION_CONFIG_FILE="${DUT_REMOTE_MORPHEUS_FOLDER}"/experiments/switch/config/adaptive_vs_naive/morpheus-adaptive-overhead.yaml
NAIVE_OPT_INSTRUMENTATION_CONFIG_FILE="${DUT_REMOTE_MORPHEUS_FOLDER}"/experiments/switch/config/adaptive_vs_naive/morpheus-naive-opt.yaml

function show_help() {
usage="$(basename "$0") [-h] [-r #runs]
Run Switch tests with different traffic localities using remote server
