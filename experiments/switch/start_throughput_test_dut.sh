...
Run L2 switch program and configure it
...
echo -e "${COLOR_YELLOW}[ DUT ] Creating Switch service (br1).${COLOR_OFF}"
if [ -z ${MORPHEUS_ENABLED+x} ]; then
  sudo polycubectl bridge add br1 type=xdp_drv stp-enabled=false loglevel=off dyn-opt=false
else
  sudo polycubectl bridge add br1 type=xdp_drv stp-enabled=false loglevel=off dyn-opt=true
fi
echo -e "${COLOR_GREEN}[ DUT ] Polycube Switch service created.\n${COLOR_OFF}"

echo -e "${COLOR_YELLOW}[ DUT ] Attaching ports.${COLOR_OFF}"
sudo polycubectl br1 ports add p1 peer=${IF1_NAME}
sudo polycubectl br1 ports add p2 peer=${IF2_NAME}
echo -e "${COLOR_GREEN}[ DUT ] Ports attached.\n${COLOR_OFF}"

if [ -n "$LATENCY_RULESET" ]; then
  # For load traffic
  sudo polycubectl br1 fdb entry add 1 ${PKTGEN_MAC_IF2} port=p2
  # For latency traffic
  sudo polycubectl br1 fdb entry add 1 12:34:56:78:90:00 port=p2
fi
...
if [ -z ${MORPHEUS_ENABLED+x} ]; then
  ...
  set -x
  sudo polycubectl br1 set start-morpheus=true
fi
