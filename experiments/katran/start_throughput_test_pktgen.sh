...
if [ $test_name == "high_loc" ]; then
  PCAP_FILE_NAME=${PCAP_PATH}/katran_high_loc.pcap
elif [ $test_name == "low_loc" ]; then
  PCAP_FILE_NAME=${PCAP_PATH}/katran_low_loc.pcap
elif [ $test_name == "no_loc" ]; then
  PCAP_FILE_NAME=${PCAP_PATH}/katran_no_loc.pcap
else
...
Run Katran test with different traffic localities
...
echo -e "${COLOR_GREEN}[ PKTGEN ] Starting Katran test: ${TEST_NAME} ${COLOR_OFF}"
...
sudo mv -f results_port_0.csv ${RESULTS_FOLDER_PATH}/${OUT_FILE} &> /dev/null

echo -e "${COLOR_GREEN}[ PKTGEN ] Katran test: ${TEST_NAME} completed ${COLOR_OFF}"
