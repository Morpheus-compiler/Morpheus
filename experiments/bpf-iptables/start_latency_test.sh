...
IPTABLES_LATENCY_SCRIPT_ORIGIN="${DIR}"/config/latency/latency-bpf-iptables.lua
IPTABLES_LATENCY_SCRIPT_NON_OPT_ORIGIN="${DIR}"/config/latency/latency-bpf-iptables-non-opt.lua

IPTABLES_LATENCY_SCRIPT_NEW="${DIR}"/config/latency/latency-bpf-iptables_new.lua
IPTABLES_LATENCY_SCRIPT_NON_OPT_NEW="${DIR}"/config/latency/latency-bpf-iptables-non-opt_new.lua
...
Run BPF-iptables latency tests for optimized and unoptimized path
...
function cleanup {
  set +e
  rm ${IPTABLES_LATENCY_SCRIPT_NEW} &> /dev/null
  rm ${IPTABLES_LATENCY_SCRIPT_NON_OPT_NEW} &> /dev/null
...
cp ${IPTABLES_LATENCY_SCRIPT_ORIGIN} ${IPTABLES_LATENCY_SCRIPT_NEW} 
sed -i -e 's/<PKTGEN_MAC_IF1>/'${PKTGEN_MAC_IF1}'/g' ${IPTABLES_LATENCY_SCRIPT_NEW}
sed -i -e 's/<PKTGEN_MAC_IF2>/'${PKTGEN_MAC_IF2}'/g' ${IPTABLES_LATENCY_SCRIPT_NEW}
sed -i -e 's/<DUT_MAC_IF1>/'${DUT_MAC_IF1}'/g' ${IPTABLES_LATENCY_SCRIPT_NEW}

cp ${IPTABLES_LATENCY_SCRIPT_NON_OPT_ORIGIN} ${IPTABLES_LATENCY_SCRIPT_NON_OPT_NEW} 
sed -i -e 's/<PKTGEN_MAC_IF1>/'${PKTGEN_MAC_IF1}'/g' ${IPTABLES_LATENCY_SCRIPT_NON_OPT_NEW}
sed -i -e 's/<PKTGEN_MAC_IF2>/'${PKTGEN_MAC_IF2}'/g' ${IPTABLES_LATENCY_SCRIPT_NON_OPT_NEW}
sed -i -e 's/<DUT_MAC_IF1>/'${DUT_MAC_IF1}'/g' ${IPTABLES_LATENCY_SCRIPT_NON_OPT_NEW}
...
for l in "${load[@]}"; do
  ...
  for i in $(eval echo "{1..$NUMBER_RUNS}"); do
    ...
    sudo ${MOONGEN_BUILD_DIR}/MoonGen ${IPTABLES_LATENCY_SCRIPT_NEW} 0 1 ${rate} 0.1 ${TEST_DURATION}
    popd

    sudo mv ${MOONGEN_BUILD_DIR}/../iptables-latency.txt ${RESULTS_FOLDER_PATH}/results_baseline_latency_${l}_run${i}.csv
  done
  ...
  for test in "${test_type[@]}"; do
    ...
    for i in $(eval echo "{1..$NUMBER_RUNS}"); do
      ...
      if [ $test == "optimized" ]; then
        sudo ${MOONGEN_BUILD_DIR}/MoonGen ${IPTABLES_LATENCY_SCRIPT_NEW} 0 1 ${rate} 0.1 ${TEST_DURATION}
        sudo mv ${MOONGEN_BUILD_DIR}/../iptables-latency.txt ${RESULTS_FOLDER_PATH}/results_${test}_latency_${l}_run${i}.csv
      else
        sudo ${MOONGEN_BUILD_DIR}/MoonGen ${IPTABLES_LATENCY_SCRIPT_NON_OPT_NEW} 0 1 ${rate} 0.1 ${TEST_DURATION}
        sudo mv ${MOONGEN_BUILD_DIR}/../iptables-latency-non-opt.txt ${RESULTS_FOLDER_PATH}/results_${test}_latency_${l}_run${i}.csv
      fi
      popd
    done
  done
