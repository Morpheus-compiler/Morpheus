...
local TS_SRC_IP	    = "153.90.25.25"
local TS_DST_IP		= "180.204.131.235"
local TS_SRC_PORT	= 7648
...
function txTimestamper(queue, time, tsLoad)
	...
	local rateLimit = timer:new(tsLoad)
	local runtime = nil
	if time > 0 then
...
function rxTimestamper(queue)
	...
	local f = io.open("iptables-latency.txt", "w+")
