...
if [ $test_name == "high_loc" ]; then
  PCAP_FILE_NAME=${PCAP_PATH}/router_high_loc.pcap
elif [ $test_name == "low_loc" ]; then
  PCAP_FILE_NAME=${PCAP_PATH}/router_low_loc.pcap
elif [ $test_name == "no_loc" ]; then
  PCAP_FILE_NAME=${PCAP_PATH}/router_no_loc.pcap
else
...
Run Router test with different traffic localities
...
echo -e "${COLOR_GREEN}[ PKTGEN ] Starting Router test: ${TEST_NAME} ${COLOR_OFF}"
...
echo -e "${COLOR_GREEN}[ PKTGEN ] Router test: ${TEST_NAME} completed ${COLOR_OFF}"
