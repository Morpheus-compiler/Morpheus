...
ROUTER_LATENCY_SCRIPT_ORIGIN="${DIR}"/config/latency/latency-router.lua
ROUTER_LATENCY_SCRIPT_NON_OPT_ORIGIN="${DIR}"/config/latency/latency-router-non-opt.lua

ROUTER_LATENCY_SCRIPT_NEW="${DIR}"/config/latency/latency-router_new.lua
ROUTER_LATENCY_SCRIPT_NON_OPT_NEW="${DIR}"/config/latency/latency-router-non-opt_new.lua
...
function cleanup {
  set +e
  rm ${ROUTER_LATENCY_SCRIPT_NEW} &> /dev/null
  rm ${ROUTER_LATENCY_SCRIPT_NON_OPT_NEW} &> /dev/null
...
cp ${ROUTER_LATENCY_SCRIPT_ORIGIN} ${ROUTER_LATENCY_SCRIPT_NEW} 
sed -i -e 's/<PKTGEN_MAC_IF1>/'${PKTGEN_MAC_IF1}'/g' ${ROUTER_LATENCY_SCRIPT_NEW}
sed -i -e 's/<PKTGEN_MAC_IF2>/'${PKTGEN_MAC_IF2}'/g' ${ROUTER_LATENCY_SCRIPT_NEW}
sed -i -e 's/<DUT_MAC_IF1>/'${DUT_MAC_IF1}'/g' ${ROUTER_LATENCY_SCRIPT_NEW}

cp ${ROUTER_LATENCY_SCRIPT_NON_OPT_ORIGIN} ${ROUTER_LATENCY_SCRIPT_NON_OPT_NEW} 
sed -i -e 's/<PKTGEN_MAC_IF1>/'${PKTGEN_MAC_IF1}'/g' ${ROUTER_LATENCY_SCRIPT_NON_OPT_NEW}
sed -i -e 's/<PKTGEN_MAC_IF2>/'${PKTGEN_MAC_IF2}'/g' ${ROUTER_LATENCY_SCRIPT_NON_OPT_NEW}
sed -i -e 's/<DUT_MAC_IF1>/'${DUT_MAC_IF1}'/g' ${ROUTER_LATENCY_SCRIPT_NON_OPT_NEW}
...
for l in "${load[@]}"; do
  ...
  for i in $(eval echo "{1..$NUMBER_RUNS}"); do
    ...
    sudo ${MOONGEN_BUILD_DIR}/MoonGen ${ROUTER_LATENCY_SCRIPT_NEW} 0 1 ${rate} 0.1 ${TEST_DURATION}
    popd

    sudo mv ${MOONGEN_BUILD_DIR}/../router-latency.txt ${RESULTS_FOLDER_PATH}/results_baseline_latency_${l}_run${i}.csv
  done
  ...
  for test in "${test_type[@]}"; do
    ...
    for i in $(eval echo "{1..$NUMBER_RUNS}"); do
      ...
      if [ $test == "optimized" ]; then
        sudo ${MOONGEN_BUILD_DIR}/MoonGen ${ROUTER_LATENCY_SCRIPT_NEW} 0 1 ${rate} 0.1 ${TEST_DURATION}
        sudo mv ${MOONGEN_BUILD_DIR}/../router-latency.txt ${RESULTS_FOLDER_PATH}/results_${test}_latency_${l}_run${i}.csv
      else
        sudo ${MOONGEN_BUILD_DIR}/MoonGen ${ROUTER_LATENCY_SCRIPT_NON_OPT_NEW} 0 1 ${rate} 0.1 ${TEST_DURATION}
        sudo mv ${MOONGEN_BUILD_DIR}/../router-latency-non-opt.txt ${RESULTS_FOLDER_PATH}/results_${test}_latency_${l}_run${i}.csv
      fi
      popd
    done
  done
